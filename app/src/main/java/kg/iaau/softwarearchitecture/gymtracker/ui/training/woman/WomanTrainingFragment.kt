package kg.iaau.softwarearchitecture.gymtracker.ui.training.woman
...
import kg.iaau.softwarearchitecture.gymtracker.utils.FEMALE
import kg.iaau.softwarearchitecture.gymtracker.utils.MALE

@AndroidEntryPoint
class WomanTrainingFragment : Fragment(), TrainingSectionListener {
    ...
    private fun observeLiveData() {
        vm.trainingSectionLiveData.observe(viewLifecycleOwner, { sections ->
            sections?.let {
                adapter.submitList(it.filter { section -> section.gender == FEMALE })
            }
        })
    }
